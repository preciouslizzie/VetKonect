...
class LoginController extends GetxController {
  ...
  Future<void> loginFunction() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && parsedResponse is Map<String, dynamic>) {
        if (parsedResponse.containsKey('email')) {
          Get.snackbar('Success', 'Login Successful');
          Get.offAllNamed('/dashboard');
        } else {
          // Handle server error responses or failed login attempts
          String errorMessage = parsedResponse['message'] ?? parsedResponse['detail'] ?? 'Login failed. Please try again.';
          Get.snackbar('Error', errorMessage);
        }
      } else {
        // Handle unexpected status codes or structure
        Get.snackbar('Error', parsedResponse['detail'] ?? 'Login failed. Please check your credentials.');
      }
    } catch (e) {
